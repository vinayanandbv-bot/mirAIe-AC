...
export class PanasonicMiraieAccessory {
    ...
    constructor(platform, accessory, device) {
        ...
        this.fanModeSwitch.getCharacteristic(this.platform.Characteristic.On)
            .onSet(async (value) => {
            const displayShouldBeOn = this.displaySwitch.getCharacteristic(this.platform.Characteristic.On).value;
            if (value) {
        ...
        if (oldDryMode)
            this.accessory.removeService(oldDryMode);
    ...
    async enforceDisplayState(displayShouldBeOn) {
    ...
    getSwingSpeedFromMode(mode) {
        const s = String(mode);
        if (s === '1' || s === String(SwingMode.ONE))
            return 20;
        if (s === '2' || s === String(SwingMode.TWO))
            return 40;
        if (s === '3' || s === String(SwingMode.THREE))
            return 60;
        if (s === '4' || s === String(SwingMode.FOUR))
            return 80;
        if (s === '5' || s === String(SwingMode.FIVE))
            return 100;
        return 0; // Auto
    }
    getSwingModeFromSpeed(speed) {
        if (speed === 0)
            return SwingMode.AUTO;
        if (speed <= 20)
            return SwingMode.ONE;
        if (speed <= 40)
            return SwingMode.TWO;
        if (speed <= 60)
            return SwingMode.THREE;
        if (speed <= 80)
            return SwingMode.FOUR;
